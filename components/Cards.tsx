...
export const FeaturedCard = ({onPress,item:{image,rating,price,address,name}}:Props) => {
  return (
...
export const Card = ({onPress,item:{image,rating,price,address,name}}:Props) => {
  return (
    <TouchableOpacity onPress={onPress} className='flex-1 w-full mt-4 px-3 py-4 rounded-lg bg-white shadow-lg shadow-black-100/70 relative'>
      <View className='flex flex-row items-center absolute px-2 right-5 top-5 bg-white/90 p-1 rounded-full z-50'>
            <Image source={icons.star} className='size-3.5'/>
